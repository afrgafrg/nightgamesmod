    ...
    public Movement execute(Character user) {
        if (user.human()) {
            if (user.location().name.equals("Showers")) {
                Global.global.gui().message("It's a bit dangerous, but a shower sounds especially inviting right now.");
            } else if (user.location().name.equals("Pool")) {
                Global.global.gui().message("There's a jacuzzi in the pool area and you decide to risk a quick soak.");
            }
        }
