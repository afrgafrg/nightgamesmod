...
public class Cache implements Deployable {
    ...
    public void gainMoney(Character active) {
        ...
        if (active.human()) {
            Global.global.gui().message("You have found " + reward + "$!");
        }
    }
    ...
    public boolean resolve(Character active) {
        if (active.state == State.ready) {
            ...
            if (active.check(test, dc)) {
                if (active.human()) {
                    switch (test) {
                        case Cunning:
                            ...
                            break;
                        case Perception:
                            Global.global.gui().message("<br/><br/>Something is off in this " + Global.global.getMatch()
                                            .genericRoomDescription()
                ...
                active.modMoney(Global.global.random(500) + 500);
    ...
    public void calcReward(int level) {
        int value = level + Global.global.random(10);
        switch (value) {
            case 23:
