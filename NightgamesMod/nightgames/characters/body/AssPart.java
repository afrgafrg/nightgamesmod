...
public class AssPart extends GenericBodyPart {
        ...
        if (self.has(Trait.autonomousAss)) {
            c.write(self, Formatter.format(
                            "{self:NAME-POSSESSIVE} " + fullDescribe(self)
                                            ...
                                            + "seemingly with a mind of its own. {self:POSSESSIVE}"
                                            + " internal muscles feel like a hot fleshy hand inside {self:possessive}"
                                            + " asshole, jerking {other:possessive} shaft.",
                            self, opponent));
    ...
    public double applyReceiveBonuses(Character self, Character opponent, BodyPart target, double damage, Combat c) {
        ...
        if ((opponent.has(Trait.asshandler) || opponent.has(Trait.anatomyknowledge)) && opponent.canRespond() && c.getStance().mobile(opponent)) {
            c.write(opponent,
                            Formatter.format("{other:NAME-POSSESSIVE} expert handling of {self:name-possessive} ass causes {self:subject} to shudder uncontrollably.",
                                            self, opponent));
