        ...
        if (player.human()) {
            if (Flag.checkFlag(Flag.blackMarketPlus)) {
                if (!Flag.checkFlag(Flag.metRin)) {
                    ...
                    GUI.gui.message("You ask Rin about the assorted tomes and unpleasant looking idols she's laid out. <i>\"You should be careful with those, they're all "
                    ...
                    if (player.money >= 1000 * (player.getPure(Attribute.Dark) + 1)) {
                        ...
                        GUI.gui.message("You can't afford the artifacts.");
                    }
                } else if (choice.equals("S&M Gear")) {
                    GUI.gui.message("Rin opens a case containing a variety of S&M toys and bondage gear. <i>\"You probably shouldn't touch those,\"</i> she says as you reach for them. "
                    ...
                    if (player.money >= 1000 * (player.getPure(Attribute.Fetish) + 1)) {
                        ...
                        GUI.gui.message("You can't afford that.");
                    }
                } else {
                    GUI.gui.message("Ridley leaves the black market as soon as he sees you. It seems you're a preferred client now, and you can deal with Rin directly. Rin gives "
                ...
                if (!trained) {
                    if (!Flag.checkFlag(Flag.darkness)) {
                        ...
                        choose("Dark Power: $" + 1000 * (player.getPure(Attribute.Dark) + 1), nextChoices);
                    }
                    if (!Flag.checkFlag(Flag.fetishism)) {
                        choose("S&M Gear", nextChoices);
                    } else {
                        choose("Fetishism: $" + 1000 * (player.getPure(Attribute.Fetish) + 1), nextChoices);
                    }
                }
            } else {
                GUI.gui.message("You knock on the door to the room Aesop pointed you to. A somewhat overweight man, a few years older than you, looks you over for a moment "
