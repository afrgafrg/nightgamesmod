...
public class Informant extends Activity {
    ...
    public void visit(String choice, int page, List<LabeledValue<String>> nextChoices, ActivityInstance instance) {
        ...
        if (!Flag.checkFlag(Flag.metBroker)) {
            GUI.gui.message("It's almost strange seeing the Quad bustling with people knowing how completely deserted it is at night. Of course, you've been here more "
            ...
            GUI.gui.message("The spot where you're suppose to meet Aesop is quiet again. Given the number of people in the Quad during the day, you're starting to feel like people "
        ...
        if (choice.equals("The Competition")) {
            GUI.gui.message("<i>\"Looking for advice about girls? If you don't have any older brothers to ask, I might be able to help.\"</i><br/>Aesop laughs a bit at his own crappy joke.<br/><i>\"In all seriousness, you should definitely "
        ...
        if (choice.equals("More Competitors")) {
            if (!Flag.checkFlag(Flag.Reyka) && Flag.checkFlag(Flag.blackMarketPlus)) {
                GUI.gui.message("<i>\"Let me tell you the story of a succubus named Reyka. So a while back a competitor decides it's a good idea to save up a bunch of money and buy the most "
                ...
                choose("Reyka: $1000", nextChoices);
            }
            if (!Flag.checkFlag(Flag.Airi) && Flag.checkFlag(Flag.workshop)) {
                GUI.gui.message("<i>\"So there's a second year bio student that was involved with a bit of an accident last year. Aya? No... Arin? No... Oh yes, her name was Airi! "
                                + "She's a pretty mousey little Japanese girl, but she got along well enough. Anyways, back to her accident. I think her experiement was supposed to to "
                ...
                choose("Airi: $1000", nextChoices);
            }
            if (!Flag.checkFlag(Flag.Kat) && Flag.checkFlag(Flag.magicstore)) {
                GUI.gui.message("<i>\"So last year Kat (funny name, you'll see why later) was responsible for a couple of all time firsts. The first 'first'.... The first unprecedented event "
