...
public class MagicTraining extends Activity {
    ...
    public void visit(String choice) {
        ...
        if (!Flag.checkFlag(Flag.metAisha)) {
            ...
            choose("Lesson: $" + 1000 * (player.getPure(Attribute.Arcane) + 1), GUI.gui);
            ...
            choose("Get Animal Spirit", GUI.gui);
            choose("Lesson: $" + (500 + 500 * (player.getPure(Attribute.Arcane) + 1)), GUI.gui);
            choose("Leave", GUI.gui);
...
//                budget -= 1000 * (npc.getPure(Attribute.Arcane) + 1);
    ...
    private void presentOptions() {
        ...
        if (player.getPure(Attribute.Animism) >= 1) {
            choose("Animism training: $" + (500 + 500 * (player.getPure(Attribute.Animism) + 1)), GUI.gui);
        }
        ...
        if (player.getPure(Attribute.Arcane) >= 2 && player.money >= 200) {
            choose("Buy a minor scroll: $200", GUI.gui);
        }
        choose("Leave", GUI.gui);
    }
