    ...
    public void subVisit(String choice, List<LabeledValue<String>> nextChoices) {
        if (choice.equals("Confront about control")) {
            if (npc.getAffection(player) == 0) {
                ...
                GUI.gui.message("<i>\"Haha, yes! Great isn't it? Hands up!\"</i> You try to resist, "
        ...
        switch (choice) {
            case "Sex":
                if (npc.getAffection(player) >= 8 && (!player.has(Trait.ticklemonster) || Random.random(2) == 1)) {
                    ...
                    GUI.gui.message("You suggest finding a bed for some private fun. Mara smiles at you mischievously. <i>\"We don't really need a bed for that\"</i> She clears some space "
            ...
            case "Games":
                if (npc.getAffection(player) >= 16 && (!player.has(Trait.spider) || Random.random(2) == 1)) {
                    ...
                    GUI.gui.message("You knew Mara was clever, but you didn't expect playing a board game against her to be this frustrating. Every move you make, she seems to have planned far "
