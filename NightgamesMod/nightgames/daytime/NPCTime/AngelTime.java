...
public class AngelTime extends BaseNPCTime {
    public AngelTime(Player player) {
        ...
        loveIntro = "You meet Angel at her room, but for once, she doesn't seem eager to get to sex. You can tell she has something on her mind, so you let her lean "
                        ...
                        + "You can see how that might make some " + Global.global.getPlayer().guyOrGirl() + "s uncomfortable. The fact that Angel and Mei occasionally have sex could probably also be a point of contention. It's probably "
                        ...
                        + "need me as much as I need them. Caroline is good at making friends, but she chooses to stick with us anyway. If I have to choose between them or a " + Global.global.getPlayer().boyOrGirl() + "friend, "
    ...
    public void buildTransformationPool() {
        transformations = new ArrayList<>();
        {
            ...
            growCock.effect = (c, self, other) -> {
                other.body.add(new CockPart(CockPart.Size.big, CockPart.Mod.blessed));
                return true;
            };
    ...
    public void subVisitIntro(String choice) {
        if (npc.getAffection(player) > 0) {
            Global.global.gui()
                  .message("You text Angel, suggesting to meet up. She responds with a location where to meet her. When you arrive however, you find her friends waiting "
                                  ...
                                  + "Angel doesn't usually run off to fix her make-up just because she's meeting a " + Global.global.getPlayer().guyOrGirl() + ". I think she really likes you.\"</i><br/><br/>With perfect timing, Angel arrives. Caroline "
        ...
        } else if (Global.global.getPlayer()
                         .checkAddiction(AddictionType.ZEAL)) {
            Global.global.gui()
            ...
            Global.global.gui()
                  .message("Using the information you got from Aesop, you go looking for Angel. Your hope is to invite her to spend time training together and possibly sex. "
                                  ...
                                  + "to look like they aren't watching you. The third is simply staring openly. Do they think you're Angel's " + Global.global.getPlayer().boyOrGirl()+ "friend? <i>\"Oh, I told them we were fuck-buddies,\"</i> Angel "
            ...
            if (npc.getAffection(player) == 0) {
                ...
                Global.global.gui()
                      .message("You feel very nervous approaching Angel like this. A growing part of you feels a need to "
                                      ...
                                      + "" + Global.global.getPlayer().guyOrGirl() + " comes along, asking to worship her?\"</i> You give a start at the word 'worship',"
                                      + " but then, it does ring kind of true... <i>\"Oh, yes I am sure I can think of some nice"
                                      + " tasks for you. Hmm, I might enjoy this. Follow me, altar " + Global.global.getPlayer().boyOrGirl() + ".\"</i> Whatever confusion"
            ...
            if (npc.getAffection(player) >= 12 && (!player.hasTrait(Trait.experttongue) || Rng.rng.random(2) == 1)) {
                Global.global.gui()
                      .message("You're in Angel's room, naked and feeling a little overwhelmed. She embraces your from behind and you can feel her soft, heavy breasts pressed against "
                                      ...
                                      + "weren't kissing my " + Global.global.getPlayer().boyOrGirl() + " on my bed a minute ago? He made you orgasm and you even drank his cum.\"</i> Angel walks over to the refrigerator in the corner, gets a couple ice cubes from the "
            ...
            if (npc.getAffection(player) >= 8 && (!player.hasTrait(Trait.pokerface) || Rng.rng.random(2) == 1)) {
                Global.global.gui()
                      .message("Today all the girls left the responsibility of choosing a game to you, with Angel's stipulation that it had to including stripping. You picked a simple, but intense "
                                      ...
                                      + "Caroline looks you over slowly while deep in thought. <i>\"As the victor, I can have my way with the losers, but... I think I'll leave Angel's " + Global.global.getPlayer().boyOrGirl() + " to her.\"</i> Angel grins and pats the couch next to "
                ...
                Global.global.gui()
                      .message("You know this can't end well for you. When you suggested playing a game, all of the girls shared a conspiratorial smile and Angel declared she had a game in mind. "
                                      ...
                                      + "speaks up while she's clearing a spot on the floor. <i>\"It's been awhile since we got to play this. We need to find more interesting " + Global.global.getPlayer().boyOrGirl() + "s.\"</i> Sarah and Angel return holding a "
            ...
            if (npc.getAffection(player) >= 16 && (!player.hasTrait(Trait.disciplinarian) || Rng.rng.random(2) == 1)) {
                ...
                Global.global.gui()
                      .message("You and Angel borrow a small fitness room and some wrestling mats for some sparring practice. She suggests clothing removal as the victory condition instead of "
                                      ...
                                      + "bit more sympathetic, though her cheeks are tinted with a light blush. <i>\"Ouch, poor " + Global.global.getPlayer().boyOrGirl() + ". That's probably checkmate.\"</i> Sarah says nothing, but is flushed with arousal and has a hand between "
    ...
    public Optional<String> getAddictionOption() {
        return Global.global.getPlayer().checkAddiction(AddictionType.ZEAL) ? Optional.of("Worship") : Optional.empty();
    }
}
