...
public class CassieTime extends BaseNPCTime {
    public CassieTime(Character player) {
        super(player, Global.global.getNPC("Cassie"));
        ...
        loveIntro = "You and Cassie lay together in her bed while she casts spells above you. Every twitch of her fingers brings a new burst of light and color. She weaves "
                        ...
                        + "of being intimate with the " + Global.global.getPlayer().boyOrGirl() + " I liked. Most of all, I was scared that you might look down on me when you found out what a horny girl I am.\"</i> She grasps your hand and you "
    ...
    public void buildTransformationPool() {
        transformations = new ArrayList<>();
        {
            ...
            growCock.effect = (c, self, other) -> {
                ...
                other.body.add(new CockPart(CockPart.Size.big, CockPart.Mod.runic));
                return true;
            };
        ...
        if (npc.getAffection(player) > 0) {
            ...
            Global.global.gui().message(
                            "You're in the library, looking around to see if Cassie is around. Soon you spot her entering a private study room. When you follow her in, she jumps "
                                            ...
                                            + "around her and kissing her tenderly. <i>\"Hanging out with a cute " + Global.global.getPlayer().boyOrGirl() + " interspersed with wild sex games? Sounds like a great date,\"</i> she says, face flushed but "
            ...
            Global.global.gui().choose(this, "Sparring");
            Global.global.gui().choose(this, "Sex");
            if (npc.hasTrait(Trait.magicmilk) && Global.global.getPlayer().getAddictionSeverity(AddictionType.MAGIC_MILK) != Severity.NONE) {
                Global.global.gui().choose(this, "Ask for milk");
            }
        }
    ...
    public void subVisit(String choice) {
        if (choice.equals("Ask for milk")) {
            if (npc.getAffection(player) > 0) {
                String msg;
                switch (Global.global.getPlayer().getAddictionSeverity(AddictionType.MAGIC_MILK)) {
                    case HIGH:
                ...
                Global.global.gui().message("Cassie lets you drink some milk, as long as you do chores for her. This"
            ...
            if (npc.getAffection(player) >= 12 && (!player.hasTrait(Trait.silvertongue) || Rng.rng.random(2) == 1)) {
                Global.global.gui().message(
                                "Cassie eagerly invites you to her room for some intimate time. The room is quite tidy, though you're surprised to see a couple anime "
                                                ...
                                                + "<i>\"Girls are interested in sex too. I've spent a lot of nights thinking about " + Global.global.getPlayer().boyOrGirl() + "s since puberty. You're the closest I've ever had to an actual " + Global.global.getPlayer().boyOrGirl() + "friend.\"</i> So she was a virgin "
            ...
            Global.global.modCounter(Flag.CassieLoneliness, -2);
            done(true);
        }
    }
    ...
    public Optional<String> getAddictionOption() {
        return Global.global.getPlayer().checkAddiction(AddictionType.MAGIC_MILK) ?
                        Optional.of("Ask for milk") :
                        Optional.empty();
    }
}
