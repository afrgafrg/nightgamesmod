...
public class ReykaTime extends BaseNPCTime {
    ReykaTime(Player player) {
        super(player, GameState.gameState.characterPool.getNPC("Reyka"));
        knownFlag = "Reyka";
        giftedString = "\"Awww thanks!\"";
        giftString = "\"A present? You shouldn't have!\"";
        transformationOptionString = "Demonic Rituals";
        loveIntro = "[Placeholder]<br/>You head down to the chapel basement. Reyka shows you a cute smile when she "
                        + "realizes that you've come to visit.";
        transformationIntro = "<br/>Reyka seems a bit shocked at your request. <i>You want to become a demon?</i> She "
                        + "looks a bit worried when she responds. You quickly reassure her that you don't necessarily "
                        + "want the whole package, corrupted soul and all. You just want some of the fun bits that she "
                        + "had made such good use of. Seeing the conversation directed back to sex, Reyka quickly "
                        + "recovers her saucy grin, <i>\"You've been having some fun I see? Alright, I can do that for "
                        + "you, but you have to bring me some ingredients. I can't do everything myself! I'll write "
                        + "down what I can do for you. One thing though, you'll have to provide me a lot of semen as "
                        + "power for the rituals. I'll give you these magic bottles, so make sure you catch it in here "
                        + "the next time you jack off\"</i>";
    ...
    public void buildTransformationPool() {
        options = new ArrayList<>();
        {
            ...
            growCock.scene = "[Placeholder]<br/>Reyka downs the bottle of the priapus draft after channeling her dark "
                            + "magic into the talisman and attaching it to her clitoris. The two of you wait, and soon "
                            + "enough, a large demonic cock sprouts out under the talisman, ripping it off from her body.";
        ...
        }
        {
            ...
            removeCock.scene = "<br/>Reyka doesn't seem extremely pleased with your request to remove her new found "
                            + "maleness, but complies anyways with your wishes. Taking the FemDrafts you offer her, she "
                            + "drinks them one after another and waits with her cock still proudly erect. Soon enough "
                            + "though, the organ starts shrinking back into her body as if being absorbed by her lower "
                            + "lips. Finally the cock head retreats into her fleshy hood, becoming indistinguishable "
                            + "from her old clitoris.";
        ...
        demonWings.scene = "Reyka smiles and crushes the ingredients together and draws a magic formation on your back "
                        + "and shoulders. After telling you to sit down across from her, she starts masturbating. "
                        + "Dumbfounded at her sudden action, you start getting up from your chair. You quick fall off "
                        + "balance though when the markings on your back grow hot. Black demonic wings tears through "
                        + "your back and settle behind you. Engrossed with exploring your new wings, you don't even "
                        + "notice when Reyka approaches you and pushes you down. You realize that the ritual must have "
                        + "taken a lot out of her, and she looks particularly famished now. You resign yourself to be "
                        + "the demoness' partner and dinner as payment for your new wings.";
        ...
        pointedEars.addRequirement(noPartmod("ears", EarPart.pointed), "No pointed ears");
        ...
        }
        {
            ...
            devilishAss.addRequirement(level(30), "At least level 30");
        ...
        }
        {
            ...
            demonicMouth.addRequirement(level(30), "At least level 30");
    ...
    public void subVisitIntro(String choice, List<LabeledValue<String>> nextChoices) {
        if (npc.getAffection(player) > 0) {
            GUI.gui.message("You go over to the chapel, wondering if Her Demonic Highness would deign to "
                            ...
                            + "will. As you turn to go back up, Reyka appears in the doorway. <i>\"Oh hello " + player
                            .getTrueName()
            ...
            if (player.checkAddiction(AddictionType.CORRUPTION)) {
                choose("Strengthen Corruption", nextChoices);
            }
        } else if (player.checkAddiction(AddictionType.CORRUPTION)) {
            GUI.gui.message("Reyka low-affection corruption intro");
            ...
            GUI.gui.message("You were going to ask Aesop where to find Reyka, but while on your way "
            ...
            player.gainAffection(npc, 1);
            npc.gainAffection(player, 1);
            GUI.gui.message("Deciding you'd rather have lunch instead, you head for the cafeteria. Halfway there, "
    ...
    public void subVisit(String choice, List<LabeledValue<String>> nextChoices) {
        switch (choice) {
            case "Strengthen Corruption":
                if (npc.getAffection(player) == 0) {
                    GUI.gui.message("Reyka has you perform 'evil' acts to strengthen your corruption. This"
                                    + " is a placeholder. It still increases your addiction, though. You"
                                    + " won't suffer withdrawal effects tonight.");
                } else {
                    GUI.gui.message("You tell Reyka about the darkness you've been feeling inside. The thing she put there."
            ...
            case "Sex":
                if (npc.getAffection(player) >= 8 && (!player.has(Trait.desensitized) || Random.random(2) == 1)) {
                    GUI.gui.message("You hesitated for a bit too long, and again Reyka's eyes flash red. Again, your mind fogs over, "
                    ...
                    if (!player.has(Trait.desensitized)) {
                        GUI.gui.message("<br/><br/><b>Having been in the maelstrom that is Reyka's pussy while she was orgasming, you are sure "
                    ...
                    GUI.gui.message("Feeling a tad squeamish, you are about to suggest going out to get something to eat. As you turn "
            ...
            case "Sparring":
                if (npc.getAffection(player) >= 12 && (!player.has(Trait.clairvoyance) || Random.random(2) == 1)) {
                    if (!player.has(Trait.clairvoyance)) {
                        GUI.gui.message("<i>\"Are you that eager to get your ass... kicked, shall we say... again? I must "
                        ...
                        GUI.gui.message("<i>\"Ah, good. By now, you may have figured out that there really isn't that much room to grow, "
                                        ...
                                        + "down your boxers and lets you go. <i>\"Next time, "
                                        + player.boyOrGirl()
                                        + ", you're mine!\"</i> As you start the final round, three "
                    ...
                    GUI.gui.message("<i>\"So basically, you want to practice sexfighting, just more the fighting than "
            ...
            case "Games":
                if (npc.getAffection(player) >= 16 && (!player.has(Trait.locator) || Random.random(2) == 1)) {
                    ...
                    if (!player.has(Trait.locator)) {
                        GUI.gui.message("<i>\"Another game? Great! I liked the way the last one turned out!\"</i> She winks as she says this, "
                                        ...
                                        + "not what I meant, you silly " + player.boyOrGirl()
                                        + ". That would require at least five souls and I'm not allowed to harvest "
                    ...
                    GUI.gui.message("Reyka pulls you out towards the dorms. A lot of people, men and women alike, look your way, "
                                    + "jealous at the " + player.guyOrGirl()
                                    + " who is getting tugged towards the dorms by a stunningly beautiful woman "
                    ...
                    GUI.gui.message("Seeing Reyka confused is certainly a change, she is usually the very image of confidence. "
    ...
    public Optional<String> getAddictionOption() {
        return player.checkAddiction(AddictionType.CORRUPTION) ?
                        Optional.of("Strengthen Corruption") :
                        Optional.empty();
    }
}
