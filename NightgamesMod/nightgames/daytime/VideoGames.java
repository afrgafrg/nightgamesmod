...
public class VideoGames extends Activity {
    ...
    private void showScene(Scene chosen) {
        switch (chosen) {
            ...
            case mara1:
                ...
                player.gainAffection(GameState.gameState.characterPool.getNPC("Mara"), 1);
                GameState.gameState.characterPool.getNPC("Mara").gainAffection(player, 1);
            ...
            case cassiemara:
                ...
                GameState.gameState.characterPool.getNPC("Mara").gainAffection(GameState.gameState.characterPool.getNPC("Cassie"), 1);
                GameState.gameState.characterPool.getNPC("Cassie").gainAffection(GameState.gameState.characterPool.getNPC("Mara"), 1);
                break;
            case caroline1:
                GUI.gui.message(
    ...
    private Scene pickScene() {
        ...
        if (player.getAffection(GameState.gameState.characterPool.getNPC("Cassie")) >= 5) {
            available.add(Scene.cassie1);
        }
        if (player.getAffection(GameState.gameState.characterPool.getNPC("Cassie")) >= 10 && Flag.checkFlag(Flag.Cassievg1)) {
            available.add(Scene.cassie2);
        }
        if (player.getAffection(GameState.gameState.characterPool.getNPC("Mara")) >= 5) {
            available.add(Scene.mara1);
        }
        if (player.getAffection(GameState.gameState.characterPool.getNPC("Mara")) >= 10 && Flag.checkFlag(Flag.Maravg1)) {
            available.add(Scene.mara2);
        }
        if (GameState.gameState.characterPool.getNPC("Cassie").getAffection(GameState.gameState.characterPool.getNPC("Mara")) >= 10 && Flag.checkFlag(Flag.Cassievg2)
                        && Flag.checkFlag(Flag.maravg2)) {
            available.add(Scene.cassiemara);
        }
        if (player.getAffection(GameState.gameState.characterPool.getNPC("Angel")) >= 10) {
