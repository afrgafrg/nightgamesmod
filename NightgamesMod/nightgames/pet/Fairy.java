...
public class Fairy extends Pet {
    ...
    public void caught(Combat c, Character captor) {
        if (captor.human() && (gender == Ptype.fairyfem || gender == Ptype.fairyherm)) {
            ...
            c.write(captor, Global.global.format("{other:SUBJECT-ACTION:manage|manages} to catch {self:name-do} and starts pleasuring her with the tip of {other:possessive} finger. The sensitive fae clings to the probing finger desperately as she thrashes "
        ...
        if (captor.human() && gender == Ptype.fairymale) {
            ...
            c.write(captor, Global.global.format("{other:SUBJECT} snatches {self:name-do} out of the air and flicks the fairy's little testicles with {other:possessive} finger. %s in sympathy as the tiny male curls up in the fetal position "
