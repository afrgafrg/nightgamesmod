...
public class HealRay extends ArmSkill {
    ...
    public boolean resolve(Combat c, RoboArm arm, Character owner, Character target) {
        boolean stamina = Rng.rng.random(2) == 0;
        boolean mojo = Rng.rng.random(2) == 0;
        ...
        if (stamina && mojo) {
            ...
            owner.heal(c, 10 + Rng.rng.random(20), "Heal Ray");
            owner.buildMojo(c, 5 + Rng.rng.random(15), "Heal Ray");
        } else if (stamina) {
        ...
        c.write(PetCharacter.DUMMY, Global.global.format(msg, owner, target, arm.getType()
                                                                         .getDesc(),
                        arm.getName()));
