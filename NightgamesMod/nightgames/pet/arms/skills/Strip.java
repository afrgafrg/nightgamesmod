...
public class Strip extends ArmSkill {
    ...
    public boolean resolve(Combat c, RoboArm arm, Character owner, Character target) {
        ...
        if (hasTop && hasBottom) {
            slot = Rng.rng.random(2) == 0 ? ClothingSlot.top : ClothingSlot.bottom;
        } else if (hasTop) {
            ...
            accuracy *= 1.5;
            slot = Rng.rng.pickRandom(Arrays.stream(ClothingSlot.values()).filter(s -> !target.outfit.
                            slotEmpty(s)).toArray(ClothingSlot[]::new)).get();
        }
        
        if (sub || Rng.rng.random(100) < accuracy) {
            Clothing stripped = target.outfit.getTopOfSlot(slot);
            if (stripped.is(ClothingTrait.indestructible)) {
                c.write(PetCharacter.DUMMY, Global.global.format("A %s lunges at {other:name-do}, its attachment"
                                + " grabbing for {other:possessive} %s. It fails to remove it, however."
                                , owner, target, arm.getName(), stripped.getName()));
            } else {
                c.write(PetCharacter.DUMMY, Global.global.format("A steely blur flies at {other:name-do},"
            ...
            c.write(PetCharacter.DUMMY, Global.global.format("{other:SUBJECT-ACTION:manage|manages} to dodge"
