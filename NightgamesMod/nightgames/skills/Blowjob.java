...
public class Blowjob extends Skill {
    ...
    public boolean usable(Combat c, Character target) {
        boolean canUse = !c.getStance()
                           .isBeingFaceSatBy(c, getSelf(), target)
                        && getSelf().canAct();
        return ((target.crotchAvailable() && target.hasDick() && c.getStance()
                                                                  .oral(getSelf(), target)
                        && c.getStance()
                            .front(getSelf())
                        && canUse) || (getSelf().canRespond() && isVaginal(c, target)));
    }
    ...
    public float priorityMod(Combat c) {
        float priority = 0;
        if (c.getStance()
             .penetratedBy(c, getSelf(), c.getOpponent(getSelf()))) {
            priority += 1.0f;
        }
    ...
    public boolean isVaginal(Combat c, Character target) {
        return c.getStance()
                .isPartFuckingPartInserted(c, target, target.body.getRandomCock(), getSelf(),
                                getSelf().body.getRandomPussy())
                        && !c.getOpponent(getSelf())
                             .has(Trait.strapped)
                        && getSelf().body.getRandomPussy()
                                         .moddedPartCountsAs(getSelf(), ExtendedTonguedMod.INSTANCE);
    }
    ...
    public int getMojoBuilt(Combat c) {
        if (isVaginal(c, c.getOpponent(getSelf()))) {
            return 10;
                    ...
                    .isBeingFaceSatBy(c, getSelf(), c.getOpponent(getSelf()))) {
            return 0;
        } else {
            return 5;
        }
    ...
    public int accuracy(Combat c, Character target) {
        return isVaginal(c, target) || isFacesitting(c, target) || !c.getStance()
                                                                     .reachTop(target) ? 200 : 75;
    }
    ...
    public Tactics type(Combat c) {
        if (isVaginal(c, c.getStance()
                          .getPartner(c, getSelf()))) {
    ...
    public String deal(Combat c, int damage, Result modifier, Character target) {
        ...
        if (modifier == Result.miss) {
            m = String.format("You try to take %s %s into your mouth," + " but %s manages to pull away.",
                            target.nameOrPossessivePronoun(), target.body.getRandomCock()
                                                                         .describe(target),
                            target.possessiveAdjective());
        }
        if (target.getArousal()
                  .get() < 15) {
            m = "You suck on " + target.nameOrPossessivePronoun()
                            + " flaccid little penis until it grows into an intimidating large erection.";
        } else if (target.getArousal()
                         .percent() >= 90) {
            m = String.format(
                            "%s %s seems ready to burst, so you suck on it"
                                            + " strongly and attack the glans with your tongue fiercely.",
                            target.nameOrPossessivePronoun(), target.initialGender.hasCock()
                                            ? target.body.getRandomCock()
                                                         .describe(target)
                                            : "girl-cock");
        } else if (modifier == Result.special) {
            m = "You put your skilled tongue to good use tormenting and teasing " 
                               + target.possessivePronoun() + " member.";
        } else if (modifier == Result.reverse) {
            m = String.format("With %s sitting over your face, you have no choice but "
                            + "to try to suck %s off.", target.subject(), target.directObject());
        } else {
            m = String.format("You feel a bit odd, faced with %s rigid %s, but as you lick "
                            + "and suck on it, you discover the taste is quite palatable. "
                            + "Besides, making %s squirm and moan in pleasure is well worth it.",
                            target.nameOrPossessivePronoun(),
                            target.body.getRandomCock().describe(target), target.directObject());
        }
        if (modifier != Result.miss && getSelf().body.getRandom("mouth")
                                                     .isErogenous()) {
    ...
    public String receive(Combat c, int damage, Result modifier, Character target) {
        String m = "";
        if (modifier == Result.miss) {
            m += String.format("%s tries to suck %s cock, but %s %s %s hips back to avoid %s.", getSelf().getName(),
                            target.nameOrPossessivePronoun(), target.pronoun(), target.action("pull"),
                            target.possessiveAdjective(), getSelf().directObject());
        } else if (modifier == Result.special) {
            m += String.format(
                            "%s soft lips and talented tongue work over %s dick, drawing out"
            ...
            m += String.format("%s pussy lips suddenly quiver and %s a long sinuous object wrap around %s cock. "
                            + "%s realize %s controlling %s vaginal tongue to blow %s with %s pussy! "
                            + "%s lower tongue runs up and down %s shaft causing %s to shudder with arousal.",
                            ...
                            getSelf().subjectAction("are"), getSelf().possessiveAdjective(),
            ...
            m += String.format(
                            "Faced with %s dick sitting squarely in front of %s face, %s"
                                            + " obediently tongues %s cock in defeat.",
            ...
            m += String.format("%s %s soft penis into %s mouth and sucks on it until it hardens.",
                         ...
                         .percent() >= 90) {
            m += String.format(
                            "%s up the precum leaking from %s cock and %s the entire length into %s mouth, sucking relentlessly.",
            ...
            if (r == 0) {
                m += String.format(
                                "%s %s tongue up the length of %s dick, sending a jolt of pleasure up %s spine. "
                ...
                m += String.format("%s on the head of %s cock while %s hand strokes the shaft.",
                                getSelf().subjectAction("suck"), target.nameOrPossessivePronoun(),
                                getSelf().possessiveAdjective());
            } else if (r == 2) {
        ...
        if (modifier != Result.miss && getSelf().body.getRandom("mouth")
                                                     .isErogenous()) {
            m += String.format(
                            "<br/>Unfortunately for %s, as %s mouth fucks %s cock %s sensitive"
