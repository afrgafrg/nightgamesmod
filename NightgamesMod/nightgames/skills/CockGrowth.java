...
public class CockGrowth extends Skill {
    ...
    public boolean resolve(Combat c, Character target) {
        ...
        if (res != Result.miss) {
            ...
            CockPart part = target.body.getCockBelow(CockPart.Size.maximumSize().length);
            if (permanent) {
                if (part != null) {
                    target.body.addReplace(part.upgrade(), 1);
                } else {
                    target.body.addReplace(new CockPart(CockPart.Size.minimumSize()), 1);
                }
            } else {
                if (part != null) {
                    target.body.temporaryAddOrReplacePartWithType(part.upgrade(), 10);
                } else {
                    target.body.temporaryAddPart(new CockPart(CockPart.Size.minimumSize()), 10);
                }
            }
        }
    ...
    public String receive(Combat c, int damage, Result modifier, Character target) {
        ...
        if (modifier == Result.miss) {
            message = String.format("%s moving and begins chanting. %s feeling some "
                            ...
                            getSelf().subjectAction("stop"), Grammar.capitalizeFirstLetter(target.subjectAction("start")),
            ...
            if (modifier == Result.special) {
                message = String.format("%s moving and begins chanting. %s to feel %s clit grow hot, and start expanding! "
                                ...
                                Grammar.capitalizeFirstLetter(target.pronoun()), target.pronoun(), target.action("are", "is"),
