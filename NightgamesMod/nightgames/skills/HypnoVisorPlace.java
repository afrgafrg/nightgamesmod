...
public class HypnoVisorPlace extends Skill {
    ...
    public boolean requirements(Combat c, Character user, Character target) {
        return user.hasTrait(Trait.hypnovisor);
    }
    ...
    public boolean resolve(Combat c, Character target) {
        ...
        StringBuilder sb = new StringBuilder(Global.global.format("{self:SUBJECT-ACTION:walk|walks}"
        ...
        sb.append(Global.global.format(" {self:PRONOUN-ACTION:rummage|rummages} around a bit"
    ...
    private String addictionDesc(Combat c, Player player) {
        if (isInWithdrawal(player)) {
            return Global.global.format("<i>Ah, {other:name}. Why did you try to get away from my control?"
        ...
        switch (player.getAddictionSeverity(AddictionType.MIND_CONTROL)) {
            case HIGH:
                return Global.global.format("<i>You're doing really well so far, {other:name}!"
                                + " Still, a little refresher surely can't hurt?</i>", getSelf(), player);
            case LOW:
                return Global.global.format("<i>You're still a little reluctant to do as you"
                                + " are told, aren't you {other:name}? Well, I know just"
                                + " the thing to help fix that!</i>", getSelf(), player);
            case MED:
                return Global.global.format("<i>You know {other:name}, we both know that, deep"
            ...
            case NONE:
                return Global.global.format("<i>Calm down now, {other:name}. Everything will be fine,"
