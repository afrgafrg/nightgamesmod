...
public class LustAura extends Skill {
    ...
    public boolean resolve(Combat c, Character target) {
        ...
        target.add(c, Horny.getWithPsycologicalType(getSelf(), target, (float) (3 + 2 * getSelf().getExposure()), 3 + Random
                                        .random(3),
    ...
    public String deal(Combat c, int damage, Result modifier, Character target) {
        return Formatter.format("You allow the corruption in your libido to spread out from "
                        + "your body. {other:subject} flushes with arousal and presses "
                        + "{other:possessive} thighs together as "
                        + "the aura taints {other:direct-object}.", getSelf(), target);
    }
