...
public class MimicAngel extends Skill {
    ...
    public boolean usable(Combat c, Character target) {
        return getSelf().canRespond() && !getSelf().is(Stsflag.mimicry) && GameState.gameState.characterPool.getNPC("Angel").has(Trait.demigoddess);
    }
    ...
    public String receive(Combat c, int damage, Result modifier, Character target) {
        return Formatter.format("{self:NAME-POSSESSIVE} amorphous body jiggles violently and {self:pronoun} "
                        + "shrinks {self:possessive} body into a sphere. "
