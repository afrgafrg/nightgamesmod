...
public class Struggle extends Skill {
    ...
    private boolean struggleBound(Combat c, Character target) {Bound status = (Bound) target.getStatus(Stsflag.bound);
            ...
            return true;    } else {
                    if (getSelf().human()) {
                        ...
                        getSelf().subject(), target.nameOrPossessivePronoun(),
                                                    ...
                                                    getSelf().possessiveAdjective()));} else {
                            c.write(getSelf(),
                                            String.format("%s tries to squirm away, but %s better leverage.",
        ...
        if (getSelf().check(Attribute.Power,
                    ...
                    if (getSelf().hasStatus(Stsflag.cockbound)) {
                        ...
                        c.write(getSelf(),
                                        Global.global.format("With a strong pull, {self:subject} somehow managed to wiggle out of {other:possessive} iron grip on {self:possessive} dick. "
                    ...
                    if (knotted) {
                        c.write(getSelf(),
                                        Global.global.format("{self:subject} somehow {self:SUBJECT-ACTION:manage|manages} to force {other:possessive} knot through {self:possessive} tight opening, stretching it painfully in the process.",
                                                        getSelf(), target));
                        getSelf().removeStatus(Stsflag.knotted);
                    ...
                    if (!reversedStance) {
                        c.write(getSelf(),
                                        Global.global.format("{self:SUBJECT-ACTION:manage|manages} to shake {other:direct-object} off.",
                                                        getSelf(), target));
                        c.setStance(new Neutral(getSelf(), target));
                    }
            return true;    } else {
                    if (getSelf().hasStatus(Stsflag.cockbound)) {
                        CockBound s = (CockBound) getSelf().getStatus(Stsflag.cockbound);
                        c.write(getSelf(),
                                        Global.global.format("{self:SUBJECT-ACTION:try|tries} to escape {other:possessive} iron grip on {self:possessive} dick. However, {other:possessive} "
                                                        + s.binding
                        ...
                        getSelf().subject(), target.subject(), getSelf().directObject(),
                                    getSelf().possessiveAdjective(), getSelf().pronoun()));} else {
