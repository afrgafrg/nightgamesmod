...
public class SubmissiveHold extends Skill {
    ...
    public boolean requirements(Combat c, Character user, Character target) {
        return (user.get(Attribute.Seduction) > 15 && user.get(Attribute.Power) >= 15) || user.hasTrait(Trait.stronghold);
    }
    ...
    public String deal(Combat c, int damage, Result modifier, Character target) {
        if (isArmLock(c.getStance())) {
            return Global.global
                            .format("You entwine {other:name-possessive} fingers with your own, holding her in position.",
                            getSelf(), target);
        } else {
    ...
    public String receive(Combat c, int damage, Result modifier, Character target) {
        if (isArmLock(c.getStance())) {
            return Global.global.format("{self:SUBJECT} entwines {other:name-possessive} fingers with {self:possessive}"
