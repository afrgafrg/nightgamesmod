        ...
        if (getSelf().hasTrait(Trait.ticklemonster) || target.roll(getSelf(), c, accuracy(c, target))) {
            if (target.crotchAvailable() && c.getStance().reachBottom(getSelf()) && !c.getStance().havingSex(c)) {
                ...
                target.body.pleasure(getSelf(), getSelf().body.getRandom("hands"), target.body.getRandom("skin"),
                                (int) getSelf().modifyDamage(type, target, 2 + Rng.rng.random(4)), bonus, c, false, this);
                target.weaken(c, (int) getSelf().modifyDamage(type, target, weak + Rng.rng.random(10, 15)));
            } else if (hastickler() && Rng.rng.random(2) == 1) {
                type = DamageType.gadgets;
                int bonus = 0;
                if (target.breastsAvailable() && c.getStance().reachTop(getSelf())) {
                    ...
                    target.body.pleasure(getSelf(), getSelf().body.getRandom("hands"), target.body.getRandom("skin"),
                                    4 + Rng.rng.random(2), bonus, c, false, this);
                }
                target.weaken(c, (int) getSelf().modifyDamage(type, target, bonus + Rng.rng.random(5, 10)));
            } else {
                ...
                int m = (int) Math.round((2 + Rng.rng.random(3)) * (.25 + target.getExposure()));
                ...
                target.body.pleasure(getSelf(), getSelf().body.getRandom("hands"), target.body.getRandom("skin"), (int) getSelf().modifyDamage(type, target, m),
                                bonus, c, false, this);
                target.weaken(c, (int) getSelf().modifyDamage(type, target, weak + Rng.rng.random(4, 7)));
            }
        } else {
