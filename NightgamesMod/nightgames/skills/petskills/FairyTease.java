    ...
    public boolean resolve(Combat c, Character target) {
        if (target.roll(getSelf(), c, accuracy(c, target))) {
            ...
            if (target.crotchAvailable() && !c.getStance().penisInserted(target) && target.hasDick()) {
                c.write(getSelf(), Global.global.format("{self:SUBJECT} hugs {other:name-possessive} dick and rubs it with "
                ...
                c.write(getSelf(), Global.global.format("{self:SUBJECT} slips into {other:name-possessive} %s and plays with "
                ...
                c.write(getSelf(), Global.global.format("{self:SUBJECT} hugs {other:name-possessive} chest and rubs {other:possessive} nipples with "
                                + "{self:possessive} entire body until {other:pronoun-action:pull|pulls} {self:direct-object} off.",
                                getSelf(), target));
                m += 5;
            ...
            c.write(getSelf(), Global.global.format("{self:SUBJECT} flies around the edge of the fight looking for an opening.", getSelf(), target));
            return false;
        }
        return true;
    }
