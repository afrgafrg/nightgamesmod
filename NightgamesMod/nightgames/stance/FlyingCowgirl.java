...
public class FlyingCowgirl extends FemdomSexStance {
        ...
        if (top.getStamina().get() < 5) {
            if (top.human()) {
                ...
                c.write(Formatter.format("{self:SUBJECT} falls to the ground and so do you. "
                                ...
                                + "you're not sure {self:pronoun} appreciates that as much as "
                                + "you do.", top, bottom));
            }
    ...
    public Position reverse(Combat c, boolean writeMessage) {
        if (bottom.body.getRandomWings() != null) {
            ...
            if (writeMessage) {
                c.write(Formatter.format("Weakened by {self:possessive} squirming, {other:SUBJECT-ACTION:fall|falls} to the ground and so {self:action:do|does} {self:name-do}. Fortunately, {other:possessive} body cushions {self:possessive} fall, but you're not sure whether {self:pronoun-action:appreciates that as much as you do|appreciate that quite as much}. "
