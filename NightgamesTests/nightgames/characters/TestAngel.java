...
public class TestAngel extends BasePersonality {
    ...
    @Override public void rest(int time) {
        if (character.rank >= 1) {
            if (!character.hasTrait(Trait.demigoddess) && character.money >= 1000) {
                advance();
            }
        }
        if (character.hasTrait(Trait.demigoddess) && !character.hasTrait(Trait.divinity)) {
